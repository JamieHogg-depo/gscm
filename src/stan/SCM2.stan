...
real ICAR1_lpdf(vector phi, int N, int[] node1, int[] node2) {
  return -0.5 * dot_self(phi[node1] - phi[node2]) 
  + normal_lpdf(sum(phi) | 0, 0.001 * N);
 }
/**
* Log probability density of the intrinsic conditional autoregressive (ICAR) model
* @param x vector of random effects
* @param C_w Sparse representation of C
* @param C_v Column indices for values in C
* @param C_u Row starting indices for values in C
...
@return Log probability density
**
To use: ICAR2_lpdf( x | C_w, C_v, C_u, offD_id_C_w, D_id_C_w, C_eigenvalues, M );
*/
real ICAR2_lpdf(
    vector x,                                              
    vector C_w , 
...
model{
	...
	// Priors
		...
		// Z_z ~ ICAR2( C_w, C_v, C_u, offD_id_C_w, D_id_C_w, C_eigenvalues, N );
