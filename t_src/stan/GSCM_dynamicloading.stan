...
// Spatial components for Leroux prior
	...
	real kappa_fixed; 				// fixed value for kappa - only used when specific_latent_rho_fixed = 3
...
parameters {    
	...
	// Loading matrix
	array[R] vector[M] Lambda_ld;   			// lower diagonal elements of Lambda
	array[R] vector<lower=0>[L] Lambda_d;   	// diagonal elements of Lambda
...
transformed parameters{
	...
	// data uncertainty
	vector[N*K] d_sd = sqrt( (2*(Y_sd_v^2)) ./ chi_draw );
	
	// factor loadings matrix 
	array[R] cholesky_factor_cov[K,L] Lambda;
	for(u in 1:R) Lambda[u] = FL(K, L, Lambda_d[u], Lambda_ld[u], latent_var_fixed); 
	...
	{
	// create k specific loading
	matrix[L,R] Lambda_kspec;
	matrix[N,K] lat;
	for(k in 1:K){
		// non-mean centered parameterisation
		...
		// k spcific loading
		for(u in 1:R) Lambda_kspec[,u] = Lambda[u,k,]';
		// latent components
		for(n in 1:N) lat[n,k] = z[n,] * Lambda_kspec * G[,n];
		// mean vector
		mu[,k] = alpha[k] + lat[,k] + epsilon[,k];
	}
	}
}
model {
	// data uncertainty prior
	chi_draw ~ chi_square(2);
	
	// generic priors
	for(u in 1:R){
		Lambda_ld[u] ~ std_normal();
		Lambda_d[u] ~ std_normal();
	}
	alpha ~ std_normal();
	...
	for(k in 1:K){
		if(specific_latent_rho_fixed == 1)
			...
			target += LCAR_lpdf( Z_epsilon[,k] | kappa_fixed, 1, C_w, C_v, C_u, offD_id_C_w, D_id_C_w, C_eigenvalues, N );
			target += normal_lpdf( sum(Z_epsilon[,k]) | 0, 0.001 * N );
		}
	}
...
generated quantities {
	...
	matrix[N,K] yrep;
	if(me == 1){
		yrep = to_matrix( normal_rng( to_vector(mu), d_sd ), N, K );
	}
	if(me == 0){ 
	...
	{
		for (nk in 1:N*K){
			if(me == 1) log_lik[nk] = normal_lpdf( Y_v[nk] | to_vector(mu)[nk], d_sd[nk] );
